...
def load_skeletons_from_dir(path, skeltype, color_table, key='pred'):
    ...
    for i in range(pred.shape[0]):
        ...
        if pid == 1:
            ...
            torso_dir = torso_vec / np.linalg.norm(torso_vec, ord=2)
            axis = np.array([0., 0., 1.])
            r = np.cross(torso_dir, axis)
            c = np.sum(axis * torso_dir)
            mat = np.asarray([[0, -r[2], r[1]],
                            [r[2], 0, -r[0]],
                            [-r[1], r[0], 0]])
            mat = np.eye(3) + mat + mat @ mat / (1 + c)
            pred_[:, :3] = pred_[:, :3] @ mat.T
            points, limbs = add_skeleton(pred_, pid=color_table[i], skeltype=skeltype, mode='ellipsold')
            create_bbox3d(scale=[0.6, 0.6, 0.8], location=pred_[2, :3], pid=color_table[i])
