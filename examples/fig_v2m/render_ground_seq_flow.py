...
if __name__ == "__main__":
    # ${blender} --python examples/fig_v2m/render_ground_seq_flow.py -- --debug
    import argparse
    ...
    fbxname = "assets/2026V2M/evaluation_original_wv_demo_koala_keyframe_seed16/epoch40/00000001_keling1_pred_seed44.fbx"
    ...
    # 中间帧设置
    num_interp_frames = 2  # 每两个关键帧之间插入的中间帧数量
    interp_alpha = 0.3     # 中间帧的透明度

    # Collect armature list for trajectory visualization (only keyframes)
    armature_list = []

    # Step 1: Load keyframes and interpolated frames
    for ii in range(num_frames):
        frame_config = config["frames"][ii]
        
        # 计算当前关键帧的x位置（保持原来的计算方式）
        x_offset_current = frame_config["x_offset"] + default_trans * ii
        
        # 加载关键帧（不透明）
        print(f"Loading keyframe {frame_config['frame']} at ii={ii}, x={x_offset_current:.2f}")
        ...
        for mesh_obj in mesh_object_list:
            set_material_i(mesh_obj, tuple(args.body), use_plastic=False, alpha=1.0)
        
        armature_list.append((armature, mesh_object_list))
        
        # 在当前关键帧和下一个关键帧之间插入中间帧
        if ii < num_frames - 1:
            next_frame_config = config["frames"][ii + 1]
            frame_start = frame_config["frame"]
            frame_end = next_frame_config["frame"]
            
            # 计算下一个关键帧的x位置
            x_offset_next = next_frame_config["x_offset"] + default_trans * (ii + 1)
            
            for jj in range(num_interp_frames):
                # 线性插值：在两个关键帧之间的空间中均匀分布
                t = (jj + 1) / (num_interp_frames + 1)
                interp_frame = int(frame_start + t * (frame_end - frame_start))
                # x位置也在两个关键帧之间线性插值
                interp_x_offset = x_offset_current + t * (x_offset_next - x_offset_current)
                
                print(f"  Loading interp frame {interp_frame} (t={t:.2f}), x={interp_x_offset:.2f}")
                armature_interp, mesh_list_interp = load_fbx_at_frame(
                    fbxname,
                    interp_frame,
                    interp_x_offset,
                    target_frame=1
                )
                for mesh_obj in mesh_list_interp:
                    set_material_i(mesh_obj, tuple(args.body), use_plastic=False, alpha=interp_alpha)
                    # 为透明材质启用blend模式
